...
- Traceability: the backend story's spec resolved cross-story decisions this story's `spec_partner` must honor rather than re-litigate — (1) `save` vs `remove` are asymmetric for a disabled provider (`remove` always succeeds if the provider row exists; only `save` is rejected), superseding this file's earlier "save/remove... is rejected" wording; (2) `manage-api-key`'s disabled-provider (not unknown-provider — unknown stays `network_error`, byte-identical to today) response uses a new `provider_disabled` `ApiKeyErrorCode` member, so this story owns the `libs/types/src/api-key-error.ts` change, the four-locale copy, and the `api-key.service.ts` mapping; (3) `generate-lesson`'s disabled-provider (not unknown, not uncurated-model — both stay `invalid_model` 422) response uses a matching new `provider_disabled` `GenerationErrorCode` member at 422, so this story also owns the `libs/types/src/lesson-generation.ts` union change, its test, four-locale copy, and the generation error mapping.
