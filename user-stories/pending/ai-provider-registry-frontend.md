...
- Traceability: the backend story's spec resolved two cross-story decisions this story's `spec_partner` must honor rather than re-litigate — (1) `save` vs `remove` are asymmetric for a disabled provider (`remove` always succeeds if the provider row exists; only `save` is rejected), superseding this file's earlier "save/remove... is rejected" wording; (2) the `manage-api-key` disabled/unknown-provider error response uses a new `provider_disabled` `ApiKeyErrorCode` member (widening the union) rather than reusing `network_error`, so this story owns the `libs/types/src/api-key-error.ts` change, the four-locale copy, and the `api-key.service.ts` mapping.
