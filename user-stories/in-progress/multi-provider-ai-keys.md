...
- Settings: CRUD list of **configured** keys only. Empty when none; **Add new provider** starts add flow. Not a fixed always-visible list of all six.
...
## Acceptance criteria
- Given settings with no saved keys, when the learner opens key settings, then they see an empty list and an **Add new provider** action.
- Given Add new provider, when the learner picks a provider not yet saved and submits a key, then that provider appears in the list as a masked “key saved” row (raw key never shown after save).
- Given a provider already saved, when the learner tries to add it again, then it is not offered in the add picker (max one key per provider).
...
- Given all six providers saved, when the learner views settings, then **Add new provider** is unavailable/hidden.
