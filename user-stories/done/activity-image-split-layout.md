...
**As a** learner
**I want** slide images to sit beside the slide body on wide screens
**so that** images don’t push the activity down and I can see image + interaction together.
...
## Acceptance criteria
- Given a slide with any valid image and a viewport wider than it is tall (tablet/desktop), when the slide renders, the title is full-width on top and under it the content row is ~50/50: image left, body/activity right.
...
- Given a landscape or square image on a wide viewport, when the slide renders, it uses the same split layout.
- Given any image on a narrow or portrait viewport (phone), when the slide renders, layout stays stacked (image above body).
...
## Notes
- Split only when the image has valid dimensions AND viewport width > height.
