...
- **Runs once, after the full review:** the final quality gate before DoD, over the feature's changed files (vs the **delivery branch**, never blind `main`) — so it covers the code the review just fixed as well as the original build. **Escalate-only:** unmet after ≤ 2 kill rounds → `ESCALATE`; never a fabricated PASS.
