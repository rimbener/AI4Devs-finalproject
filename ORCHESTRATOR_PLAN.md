...
- **Runs once, after the full review:** the final quality gate before DoD, over the feature's changed files (vs `main`) — so it covers the code the review just fixed as well as the original build.
...
- **Loop:** any survivor → back to `implementer` (write the red test that kills it) → re-run Stryker, capped at **2 rounds** (unresolved survivors → escalate & block). This single pass, after the review, is the final gate before `dod_validator`.
