...
- **Component file split** (`component-split.mdc`): non-trivial UI (organisms / complex molecules) splits into `*.tsx` (JSX + handlers) / `*.types.ts` / `use-*.ts` (local state) / `use-*.animations.ts` (**all** animation handling) / `*.helpers.ts` (pure); not the data-layer hook.
