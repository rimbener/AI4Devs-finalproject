...
- File-input handling: `expo-document-picker`'s web implementation dynamically creates/removes a bare `<input type="file">` with no `data-testid` on every call — it can't be targeted via `getByTestId`. Click "Choose file" and `page.waitForEvent('filechooser')` together (e.g. `Promise.all`), then call `.setFiles(fixturePath)` on the resulting `FileChooser`. Targeting `page.locator('input[type="file"]')` directly and calling `.setInputFiles()` loses that race essentially every time, since the input is created/removed around the click — see `.agents/rules/app-e2e.mdc` and `tests/e2e/helpers/golden-path.js`'s `chooseFileAndExtract`.
