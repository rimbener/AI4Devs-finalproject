...
## Summary
On wide (landscape) viewports **any valid slide image** renders **beside** the body instead of stacked above it, so the image no longer pushes the activity down. Layout-only, presentational, in `@helsoft/activities` (`SlideView` + `SlideImage`, height plumbing from `LessonPlayer`). Builds on the done `activity-image-sizing` story (unchanged: max width + `contain` + expand/lightbox).

Behavior → [`gherkin-scenarios.md`](./gherkin-scenarios.md) (`@s1…@s14`); tasks/slices → [`tasks.md`](./tasks.md).

## User story
- As a **learner**, I want **slide images to sit beside the slide body on wide screens**, so **images don't push the activity down and I can see the image and the interaction together**.

## UI states (`SlideView`)
| State | Trigger | Notes |
|---|---|---|
| Loading | **N/A** — URL resolution owned by `SlideImage`/`useSlideImageUrl` (unchanged). | No layout Loading UI. |
| Content — **stacked** | No image; OR invalid image dimensions; OR portrait/narrow viewport; OR height not yet measured. | Today's column title → image → body. Safe default. |
| Content — **split** | Valid image dimensions **AND** viewport `width > height` **AND** measured `availableHeight > 0`. | Full-width title; `[image \| body]` ~50/50 (`flex:1`, `gutter` gap); image `contain`+height-bounded; body own scroller. |
...
3. **Trigger = any valid image on a landscape viewport:** `validImageDimensions && window.width > window.height`. *Why:* human broadened the original portrait-only rule after validating real lesson assets; landscape and square images also split when horizontal room exists.
