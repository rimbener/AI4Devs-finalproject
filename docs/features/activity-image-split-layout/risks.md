...
- **R5 — Split image with unknown/degenerate dims.** Missing or non-positive `width`/`height` could mis-trigger split or a 0-height image. *Mitigation:* `useSlideLayout` returns `false` unless both dims > 0 (@s9).

## Product
- **R6 — Landscape-phone behavior.** A landscape phone (`width > height`) splits, including landscape/square images. *Mitigation:* explicit human decision; horizontal room is the sole viewport trigger.
