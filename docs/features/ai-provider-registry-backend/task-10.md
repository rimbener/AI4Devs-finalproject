...
## Done criteria
- [ ] Scenario s26 covered by a Deno test
- [ ] A throwing catalog read → the existing catch-all responds **502** `{ code: 'network_error' }`
- [ ] **No** Vault write and **no** `save_api_key` RPC occurs on that path
- [ ] The failure is logged through `logEvent` with the redacted shape only — never the request body,
      never the key
- [ ] No hardcoded provider list remains anywhere in `manage-api-key`
