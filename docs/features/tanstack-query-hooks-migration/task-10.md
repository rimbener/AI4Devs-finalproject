...
## Done criteria
- [ ] **Exemptions** section names all 4 with their reason:
...
- [ ] **D2** documented: the `'auth'` key prefix is reserved; `useSession` evicts every non-`auth` entry on user-id change (not on token refresh), and the eviction precedes the session write
- [ ] **D3** documented: prefer one tagged-union mutation when the public contract exposes a single `error`/`isPending` slot shared by several actions — the existing "unless … used from the same file, or … shared code" carve-out, made concrete
- [ ] **D5** documented: `isPending` is **not** a synchronous guard; TanStack does not dedupe `mutate()`, so an action that must never double-fire keeps a small entry-gate ref cleared in `onSettled`
- [ ] **D6** documented: for time-limited resources, set `staleTime` **and** `gcTime`, both derived from the resource's published TTL — `gcTime` alone does not prevent a refetch on remount
- [ ] The provider references in the opening paragraph are corrected — `ApiKeyProvider`/`ProfileProvider` no longer exist; `QueryProvider` is the only one
- [ ] `pnpm lint` green
