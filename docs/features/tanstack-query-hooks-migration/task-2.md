...
## Done criteria
- [x] Scenario(s) s5, s6, s7, s8, s9 covered by concrete tests in `use-lesson.test.ts`
- [x] Return shape byte-for-byte identical; `lesson` is `data ?? null`
- [x] `refetch` wrapped to stay `() => void` (TanStack's returns a Promise)
- [x] `use-lesson.reducer.ts` deleted; no `requestId` ref remains
- [x] Stale-response handling comes from the key changing with `id` (s9) — no manual guard
- [x] Hook calls `LessonsService` only, never a DAO (`hooks-service-dao.mdc`)
- [x] Tests adapted, not rewritten: `renderHook` wrapped in `QueryClientProvider` with a fresh `QueryClient` per test constructed with `retry: false`; synchronous post-`act` reads replaced with `await waitFor(...)`
