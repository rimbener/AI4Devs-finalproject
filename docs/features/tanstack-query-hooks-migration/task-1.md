...
## Done criteria
- [x] Scenario(s) s1, s2, s3, s4 covered by concrete tests in `use-session.test.ts`
- [x] Eviction is guarded on **user-id change**, not on every auth event (s2 proves a same-user refresh evicts nothing)
- [x] Eviction happens **before** the session write, so the new session survives (s4)
- [x] The `'auth'`-prefixed session entry is never evicted
- [x] The existing `receivedAuthEventRef` stale-resolution guard and every existing `use-session` test still pass unchanged
