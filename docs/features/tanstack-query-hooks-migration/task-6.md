...
## Done criteria
- [ ] Scenario(s) s24, s25, s26, s27, s28, s29 covered by concrete tests
- [ ] `staleTime` **and** `gcTime` both derived from `SIGNED_URL_TTL_SECONDS` (e.g. `(TTL - 60) * 1000`), both strictly under the TTL — no hard-coded `240_000`
- [ ] `url` is `data ?? null`; the hook never throws (s27)
- [ ] `isLoading` matches today exactly: `false` when the ref is absent (disabled query), `true` synchronously on the first render when a ref is present
- [ ] Stale-response handling comes from the key changing with `storagePath` (s28) — no manual guard
- [ ] `next-request-id.ts` and `next-request-id.test.ts` deleted; no importers remain anywhere
- [ ] `lesson-player.integration.test.ts` passes with the same wrapper, otherwise unedited
