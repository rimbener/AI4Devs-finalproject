...
## Done criteria
- [x] Scenario(s) s30, s31, s32, s33, s34, s35, s36 covered by concrete tests
- [x] **s33 is a new test:** two `saveAttempt` calls **in the same tick** result in exactly one service call
- [x] `isSaving` ref gates entry for **both** call sites (`saveAttempt` and `retry`) and is cleared in `onSettled`
- [x] The ref does nothing but gate — `status` and `attempt` come from the mutation, never from parallel state
- [x] `lastInput` ref deleted; `retry` replays `mutation.variables` and is a no-op when it is `undefined` (s36)
- [x] `isMounted` ref deleted; no state-update-after-unmount warning (existing test)
- [x] `saveAttempt` returns `void` (`mutate`, not `mutateAsync`) — unchanged from today
