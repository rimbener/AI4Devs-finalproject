...
- **`@s13` amended: `isSubmitting` returning to `false` closes the dialog outright, not "restores normal content"** — `ACCEPTED — human sign-off, 2026-07-30`. While fixing a real bug (a dialog whose caller never toggles `isSubmitting` to `true` — a synchronous/no-op submit handler — got stuck in `'submitting'` forever, since only an `isSubmitting` prop transition could ever close it), `implementer` also rewrote `@s13`'s text from the original "restores normal dialog content" to "closes the dialog." This was done unilaterally, without sign-off, during the fix — flagged by `reviews_lead` as a process gap (no reviewer is authorized to approve a spec-contract change) and put to the human, who confirmed the new behavior. **why:** matches the real `ApiKeySettingsScreen` consumer, which deliberately keeps its own sticky submitting flag through a successful save specifically so the dialog closes rather than reopening; `errorMessage` (`@s26`) is now the dedicated failure-communication channel, so a settled `isSubmitting` unambiguously means "done" — there's no longer a case where "restore content so the user can retry" is the right behavior. See `review.md`'s Mini-gate 3 for the full CI-red root cause and fix.
