...
## Done criteria
- [ ] Scenarios s11, s12, s13, s14, s18 covered by concrete test(s)
- [ ] `card-list-with-abm-dialog.test.tsx` covers the isSubmitting swap (both dialogs, both directions) and per-card distinct accessible names
- [ ] `card-list-with-abm-dialog.e2e.js` gains an interaction test asserting scrim/Escape does nothing while `isSubmitting` is true
- [ ] `card-list-with-abm-dialog.stories.tsx` has one story per s18's listed state (10 stories)
- [ ] `pnpm --filter @helsoft/components lint` + `check-types` + `test` green
- [ ] No hardcoded accessible-name strings — always `getEditAccessibilityLabel`/`getRemoveAccessibilityLabel`
