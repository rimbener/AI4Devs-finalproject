...
## Done criteria
- [x] Scenario(s) s13, s14 covered by concrete test(s)
- [x] On native / web <768: Settings screen shows `SignOut`; confirming clears session → login (s13, reuses `useAuth().signOut()` + existing confirm keys)
- [x] On web ≥768: `SettingsSignOut` renders nothing on the Settings screen (s14)
- [x] Component unit test `settings-sign-out.test.tsx` asserts breakpoint gating both ways
- [x] Reuses existing `SignOut` (`@helsoft/study-buddy`) uncontrolled mode + `auth.logOut*` keys — no new i18n keys
