...
- [x] `test:e2e` green where relevant — _evidence:_ components playwright incl. `web-bottom-tabs` + `web-bottom-tab-button` e2e; study-buddy e2e green
...
- [x] Labels/roles; contrast ≥ 4.5:1; touch targets ≥ 44/48; focus order; dynamic type — @s5 native selected state; web tab `accessibilityRole="tab"` + selected; labels via `t(nav.*)`; `review-slice.md` a11y OK
