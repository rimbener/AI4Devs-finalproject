...
## Goal
Remove the MVP custom `MobileBar` now that native nav is `NativeTabs` and narrow-web nav is `WebBottomTabs`. Delete the `mobile-bar/` folder (component, `.types.ts`, stories, unit test), drop its export from `organisms/index.ts`, and remove any `mobile-bar` Playwright e2e. Keep `AccountMenu` (still used by `DesktopBar`'s avatar).
