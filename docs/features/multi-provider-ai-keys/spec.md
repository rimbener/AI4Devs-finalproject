...
| Empty | no saved keys | empty message + "Add new" (`@s1`); generate → missing-key gate (`@s16`) |
...
- **5 new `@ai-sdk/*` packages + `provider→createX(apiKey)` factory** behind the existing seams; pin `npm:@ai-sdk/deepseek@3` (no v4 on npm) — **why:** first-party adapters share the `generateObject`/vision surface; callers/tests unchanged.
- **Composite PK `(user_id, provider)`** + DB `CHECK` allow-list; redefine `save/get/remove_api_key` RPCs (get/remove take `provider`); `ON CONFLICT ON CONSTRAINT user_ai_keys_pkey` + table-qualify RETURNING cols (avoid ambiguous `provider`); `GRANT SELECT` on `user_ai_keys` to `service_role` for Edge list; settings list via existing RLS `select` (no list RPC) — **why:** minimal faithful "≤1 key per provider"; DB CHECK is server-side defense beyond the TS union.
...
- **`PROVIDER_NAME_KEYS` + `API_KEY_SETTINGS_GUIDANCE_URLS`** in `@helsoft/types` (`api-key-settings.ts`) — **why:** shared brand labels + console URLs for settings UI + generate pickers without duplicating maps.
...
- **Settings → API keys nested routes** (`(tabs)/settings/` + `/settings/api-keys`); **`ApiKeySettings`** = entry button; **`ApiKeySettingsScreen`** hosts `ApiKeyManager` — **why:** keeps Settings tab lean; recovery deep-links to `/settings/api-keys`.
- **Settings UI split (atomic):** `ApiKeyManager` → molecule `ApiKeySavedList` + organism `ApiKeyFormDialog` (`ApiKeyForm` inside); `useApiKeyManager` = `useReducer` (`state.mdc`); StyleSheets private (no exported style snapshots) — **why:** one job per piece; modal close clears draft `apiKey` only (keeps `formMode`/`formProvider`).
...
- **New error code `invalid_model`** (mirrored client↔Deno + i18n) for unknown provider/model; `missing_key` for a named provider with no key; `invalid_key` unchanged; shared i18n **`error.network`** for transport failures (auth/upload/generation/api-key) — **why:** precise generation codes + one network copy.
