...
- **Settings → API keys nested routes** (`(tabs)/settings/` + `/settings/api-keys`); `ApiKeySettings` = entry button; screen hosts `ApiKeyManager` — **why:** keeps Settings tab lean; recovery deep-links to `/settings/api-keys`.
- **`ApiKeyManager`**: list + Add/Replace in modal `Dialog` (unsaved-only `RadioGroup`); empty = message + Add; Add hidden at 6/6; `ApiKeyForm` = replace sub-form — **why:** configured list + add flow without inline expand clutter.
