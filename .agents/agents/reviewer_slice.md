...
- **`hooks-service-dao.mdc`** — layering `Component → Hook → Service → DAO`; DAOs = data access only; services = validation/business logic, no React; hooks wrap services, not DAOs; each layer exports via `index.ts`. Pick `@helsoft/supabase-services` vs `@helsoft/services` by data source.
- **`tanstack-query.mdc`** — data-fetching/mutation hooks in `@helsoft/hooks` use `useQuery`/`useMutation`; single `QueryClient` via `QueryProvider` (never a second one); expose the mutation primitives directly (renamed to the action, e.g. `isSigningOut`), don't re-derive with `useState`; bridge external events with `queryClient.setQueryData` (no parallel `useState`); normalize `error` through a typed guard; tests wrap `renderHook` in `QueryClientProvider` and use `waitFor`.
- **`state-sharing.mdc`** — cross-subtree sharing via React **Context** (avoid deep / large prop-drilling); still no Redux; local coordinated state stays `useReducer` per `state.mdc`.
...
- **`component-split.mdc`** — non-trivial UI split into `*.tsx` (JSX + handlers) / `*.types.ts` / `use-*.ts` (local state) / `use-*.animations.ts` (**all** animation handling) / `*.helpers.ts` (pure); handlers stay in the component, helpers stay pure.
...
- **`types.mdc`** — a `*.types.ts` file exists **only** for a type another file imports; single-file types stay inline (flag a `*.types.ts` whose types nothing else imports). **No declared result/return types — prefer inference** (flag a `FooServiceResult`/`UseFooResult` or a redundant return annotation). No runtime logic in types files.
