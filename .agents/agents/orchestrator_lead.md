...
7. **Phase 3b — mutation.** Set `mutation` (via `set-feature-phase.sh`). Run `mutation_tester`, which uses `run-mutation.sh` scoped to the feature's changed files **vs the delivery branch** (`ORCHESTRATOR_BASE_REF`, not blind `main`), covering the code the review just fixed. **Every surviving mutant → back to `implementer`** (write the red test that kills it) → re-run until the **threshold is met (100% on changed lines)** — ≤ 2 rounds; if still unmet it is **escalate-only** (`ESCALATE`, hard) — **never rewrite `mutation.md` to a PASS or invent a `human-excluded` waiver**. This is the final quality gate before DoD.
