...
1. `spec_partner` **grills the human** (asks questions, one at a time) and **writes** `spec.md` + `tasks.md` + `task-N.md` + `gherkin-scenarios.md` (`risks.md` → gitignored `tmp/<name>/`) → `spec_reviewer` vets the bundle (`review-spec.md`) **once (1 round)**; `spec_partner` fixes every finding (no re-review; unresolvable → escalate) → `spec_ready` → **⏸ HUMAN GATE** (the pipeline's **single** approval: approve `spec.md` + `gherkin-scenarios.md` together). No separate plan approval.
...
3. After all slices — **quality gate: full review → mutation**:
   ...
   b. `mutation_tester` → StrykerJS **once** (via `run-mutation.sh`), on the feature's changed files vs the **delivery branch** (covers the review's fixes too) → `implementer` kills every survivor (prefer strengthening the **test**; ≤ 2 rounds; unresolved → **ESCALATE**, never a fabricated PASS).
   c. **Conditional re-review:** if killing survivors changed **production source** (`libs/*/src` `.ts`/`.tsx`, excluding tests/stories/e2e) → re-run the **full review** (`reviews_lead`) on that delta, fix findings (bounded, ≤ 2 rounds; re-run mutation if the review changes source). If the mutation fix was **test-only** → skip, go to DoD.
