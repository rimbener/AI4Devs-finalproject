...
describe('useActivitySubmitResultAnimations', () => {
  afterEach(() => {
    jest.restoreAllMocks();
  });

  it('exposes the slide-up offset for the entrance animation', async () => {
    const { result } = await renderHook(() => useActivitySubmitResultAnimations('submit'));
  ...
  it('replays the entrance animation whenever a visible mode appears', async () => {
    ...
    const { rerender } = await renderHook(
      ({ mode }: { mode: RenderMode }) => useActivitySubmitResultAnimations(mode),
  ...
  it('does not animate while hidden', async () => {
    ...
    await renderHook(() => useActivitySubmitResultAnimations('hidden'));
